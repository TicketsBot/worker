...
func OnDirectOpenMessageReact(worker *worker.Context, e *events.MessageReactionAdd) {
	...
	if isBlacklisted {
		utils.SendEmbed(worker, dmChannel.Id, targetGuild.Id, nil, utils.Red, "Error", translations.MessageBlacklisted, nil, 30, true)
		return
	}
	...
	utils.SendEmbed(worker, dmChannel.Id, targetGuild.Id, nil, utils.Green, "Modmail", translations.MessageModmailOpened, nil, 0, true, targetGuild.Name)
	...
	welcomeMessageId, err := utils.SendEmbedWithResponse(worker, dmChannel.Id, nil, utils.Green, "Modmail", welcomeMessage, nil, 0, true)
	if err != nil {
		utils.SendEmbedRaw(worker, dmChannel.Id, nil, utils.Red, "Error", fmt.Sprintf("An error has occurred: %s", err.Error()), nil, 30, true)
		return
	}
	...
	utils.SendEmbedRaw(worker, staffChannel, nil, utils.Green, "Modmail", welcomeMessage, nil, 0, true)
}
